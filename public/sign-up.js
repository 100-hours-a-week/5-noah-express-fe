...
function validteAll() {
    return (validProfileImageStatus && validEmailStatus && validPasswordStatus && validPasswordCheckStatus && validNicknameStatus);
}
...
const profileImageHelperText = document.getElementById('profile-image-helper-text');
...
document
    ...
    .addEventListener('change', function () {
        ...
        profileImageHelperText.innerText = validProfileImageStatus ? '' : '* 프로필 사진을 추가해주세요.';
...
emailInput.addEventListener('change', () => {
    ...
    emailHelperText.innerText = validEmailStatus ? '' : '* 올바른 이메일 주소 형식을 입력해주세요. (예: example@example.com)';
...
passwordInput.addEventListener('change', () => {
    ...
    if (value.length == 0) {
        ...
        passwordHelperText.innerText = validPasswordStatus ? '' : '*비밀번호는 8자 이상, 20자 이하이며, 대문자, 소문자, 숫자, 특수문자를 각각 최소 1개 포함해야 합니다.';
    }
...
// 비밀번호 확인 관련 로직
const passwordCheckHelperText = document.getElementById('password-check-helper-text');
...
passwordCheckInput.addEventListener('change', () => {
    ...
    if (value.length == 0) {
        ...
        passwordCheckHelperText.innerText = validPasswordCheckStatus ? '' : '* 비밀번호가 다릅니다.';
    }
